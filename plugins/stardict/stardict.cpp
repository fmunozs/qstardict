    ...
    : QObject(parent)
{
    ...
    m_reformatLists = false; settings.value("StarDict/reformatLists", false).toBool();
    m_expandAbbreviations = settings.value("StarDict/expandAbbreviations", true).toBool();
    if (m_dictDirs.isEmpty())
    {
        m_dictDirs << QDir::homePath() + "/MyDocs/dic";
...
QStringList StarDict::findSimilarWords(const QString &dict, const QString &word)
{
    ...
    {
        result << QString::fromUtf8(g_ascii_strdown(g_strstrip(*p), -1));
        g_free(*p);
    }
    return result;
}
